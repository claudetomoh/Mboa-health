...
|---|---|---|---|---|
| Implementation Master Plan v1.1 not committed to repo; repo has stale v1.0 | Critical | Open | Product | Any agent or contributor working from the repo directly sees the wrong plan. Commit v1.1 immediately. |
...
| Symptom Checker false AI claim present in two locations, not one | High | Open | Frontend | `symptom_checker_screen.dart:492` and `dashboard_screen.dart:423`. Original Task 1 scope only covered the first. Reissued. |
